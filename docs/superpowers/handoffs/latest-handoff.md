...
- Current local checkpoint:
  ...
  - local work is an unreleased follow-up to later Retail two-client sync findings and now targets `v1.3.1`; do not treat it as part of the published `v1.2.2` asset
- Additional live duplicate-money report:
  ...
  - v1.3.1 should preserve item ledger rows while cleaning only duplicate money-ledger rows and preventing fixed clients from accepting the same duplicate money rows again
...
- Local fix in progress:
  ...
  - `GBankManager.toc` and the addon version fallback now advertise `1.3.1` / `v1.3.1`
  - Guild Info auth-policy pulls now skip protected `GetInfoText()` reads while combat-locked and convert blocked reads into a quiet failure result instead of surfacing an `ADDON_ACTION_BLOCKED` Lua error during guild events
