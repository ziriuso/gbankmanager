...
git commit -m "chore: prepare 1.2.3 release"
...
```powershell
gh release view v1.2.3 --json name,tagName,isPrerelease,assets,url
```

The stable release should have `isPrerelease: false`, a `GBankManager-1.2.3.zip` asset, and a successful CurseForge upload step in the workflow log.
