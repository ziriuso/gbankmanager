...
git commit -m "chore: prepare 1.3.0 release"
git push origin codex/gbankmanager-code-review-remediation
```
...
```powershell
gh release view v1.3.0 --json name,tagName,isPrerelease,assets,url
```

The stable release should have `isPrerelease: false`, a `GBankManager-1.3.0.zip` asset, and a successful CurseForge upload step in the workflow log.
