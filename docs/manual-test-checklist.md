...
10a. Confirm Exports presents `Auctionator`, `TSM`, `CSV Spreadsheet`, and `Manual Shopping List` as distinct action cards with one-line descriptions before opening any modal output.
...
13a. Confirm the dashboard now shows four metric cards (`Last Scan`, `Pending Requests`, `Ready to Buy`, `Critical Shortages`) plus separate `Top 5 Most Used`, `Recent Activity`, and `Quick Actions` panels.
13b. Confirm each dashboard metric card now shows a large icon and still keeps the card title, primary value, and subtitle readable at the default shell scale.
...
21a. Close and reopen the guild bank after more than 10 minutes since the last successful scan and confirm the scan starts automatically without using `/gbm scan` or the `Scan Bank` button.
21b. Reopen the guild bank again inside the 10-minute window and confirm auto-scan does not restart, then trigger `/gbm scan` manually and confirm manual scan still runs immediately.
...
32c. Confirm the bottom Request Admin filter strip switches between `All`, `Pending Approval`, `Pending Fulfillment`, and `Completed`, keeps the active filter highlighted, right-aligns those filters, and keeps `Add Request` plus `Refresh` anchored on the far left.
32d. Open a deletable request in `Request Admin`, confirm the details popup exposes `Delete` only when the viewer has the request-delete permission, click it, and confirm the row disappears without Lua errors.
...
57a. Run `/gbm request` and confirm the smaller end-user workflow shows `Guild Bank Manager` in the header, an own-request table with `Item ID`, `Item Name`, `Quantity`, and `Status`, plus a `New Request` button that opens a four-step request wizard with a progress rail.
...
59ab. In `Request Admin`, confirm `All`, `Pending Approval`, `Pending Fulfillment`, and `Completed` render as tab-like pills, while `Add Request` and `Refresh` render as separate secondary action buttons instead of matching the filters.
...
59ca. In Inventory, History, Minimums, Request Admin, and Exports, confirm the shared table header, filter strip, and viewport all retain the carved mockup styling and alternating row backgrounds instead of falling back to one generic boxed panel look.
...
59fa. Confirm the manual shopping list shows the same crafted-quality icon family used elsewhere instead of raw `T2` / `T3` text prefixes, stays open when switching tabs or closing the main shell, and reopens in its last moved position.
...
59h. In `Options`, confirm the local appearance controls now include theme presets (`Generic WoW`, `High Contrast`, `Alliance`, `Horde`, `Nature`, `Void`), linked `Shell Scale` / `Table Density` behavior, and separate shell-vs-modal opacity sliders. Change each control and confirm the shell updates immediately without misaligning headers, rows, footer strips, top-bar buttons, or modal chrome.
59ha. In `Options`, drag each slider directly and confirm the updated slider chrome feels deliberate in motion: the thumb should not get stuck after releasing off the bar, the track should stay visually aligned, and the new darker `Generic WoW` baseline should match the mockup more closely than the older lighter shell.
59hb. In `Options`, confirm the top-level tabs now read `Appearance`, `Permissions`, `Blacklist`, `Automation`, `Exports`, and `Requests`, and that switching tabs swaps a single content pane instead of rendering one long stacked settings page.
...
59o. Seed stocking history across several scans for at least two minimum-tracked items and confirm the dashboard `Top 5 Most Used` card ranks repeated shortage cycles ahead of a single large withdrawal-only item.
...
60. Blacklist a guild member from the auth panel using `Character-Server` input and confirm the target officer note gains `[GBMBL]`, both `/gbm ui` and `/gbm request` deny access for that character after `/reload`, and the surrounding officer note text is preserved.
...
64. In `Options -> Auth`, click `Select All` beside the `Policy String` and confirm the whole snippet highlights with visible copy guidance.
...
66. After removing `[GBMBL]` from a guild member's officer note and refreshing guild data, confirm the addon drops the active blacklist membership while keeping any previously learned local reason available for future re-tagging.
