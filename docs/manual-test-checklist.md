...
32c. Confirm the bottom Requests filter strip switches between `All`, `Pending Approval`, `Pending Fulfillment`, and `Completed`, keeps the active filter highlighted, right-aligns those filters, and keeps `Add Request` plus `Refresh` anchored on the far left.
...
59hb. In `Options`, confirm the top-level tabs now read `Appearance`, `Stock Settings`, `Permissions`, `Blacklist`, `Automation`, `Exports`, and `Requests`, and that switching tabs swaps a single content pane instead of rendering one long stacked settings page.
...
59ie. Toggle `Show Minimap Button` off and on in `Options -> Appearance`, confirm the minimap launcher hides and reappears immediately, and confirm it uses the shipped custom minimap art instead of the old shell crest icon.
...
66d. On Dashboard, confirm longer `Quick Actions` labels such as `Request Overview` wrap cleanly inside the button instead of clipping or running into neighboring actions.
