...
27d. Confirm the Minimums table height matches Inventory and the bottom area is only a clean raised four-button action strip (`Add`, `Save All`, `Enabled Only`, `Show All`) without the old boxed footer panel or buttons hugging the bottom edge.
...
32c. Confirm the bottom Request Admin filter strip switches between `All`, `Pending Approval`, and `Pending Fulfillment`, keeps the active filter highlighted, right-aligns those filters, and keeps `Add` anchored on the far left.
...
59h. In `Options`, confirm the local appearance controls now include theme presets (`Current`, `Contrast`, `Horde`, `Alliance`, `Void`, `Adventurer`, `Moonglade`), linked `Shell Scale` / `Table Density` behavior, and separate shell-vs-modal opacity sliders. Change each control and confirm the shell updates immediately without misaligning headers, rows, footer strips, top-bar buttons, or modal chrome.
