...
- `live smoke`
  ...
  - `Options -> Data` should let you open real dropdown menus for guild-bank ledger retention, audit-history retention, and the guild-bank `Scan Interval`, should expose the repair-threshold input, should keep the save button inside the panel chrome, should show visible save feedback after `Save Settings`, should surface a `Clear Data` section with confirmation-gated destructive actions, and saving those settings should prune out-of-window Bank Ledger item/money rows plus audit-history rows when retention is finite while leaving inventory snapshots, Minimums, Requests, and sync peers alone. The immediate local toggles under `Options -> Appearance`, including `Mute Silvermoon Citizen` and `Suppress Routine Chat`, should persist after `/reload`.
  ...
  - sync should report compact chat feedback for accepted incoming updates and rejected forged sync payloads without turning the chat frame into a step-by-step spam log, while self login hello stays silent
...
- `tests/spec/sync_spec.lua` now also verifies sync hello traffic and request snapshots can bootstrap a client whose root still points at the placeholder `Unknown Guild`, canonicalize stored peer identities to `Character-Server`, and promote the live guild key before peers are filed or request snapshots are rejected as `wrong_guild`.
...
- `tests/spec/sync_spec.lua` now also verifies duplicate remote ledger deltas that merge zero new rows are still accepted for peer bookkeeping but do not add another `Synced ledger delta ...` chat line, and that ledger deltas without a same-or-newer addon version are rejected before they can mutate the repaired ledger.
- `tests/spec/sync_ledger_digest_spec.lua` verifies ledger sync builds a compact digest, announces it before row deltas, suppresses repeated same-hash delta sends inside the burst window, and records matching inbound `LEDGER_DIGEST` traffic as converged sync state.
...
- `tests/spec/sync_spec.lua` now also verifies accepted remote request snapshots reconstruct the existing local `REQUEST_*` History rows when a snapshot is what catches a client up, accepted remote minimum snapshots reconstruct the existing local `MINIMUM_*` History rows while staying quiet on identical replays, accepted remote `HISTORY_SNAPSHOT` payloads merge only the visible History-tab rows without duplicating identical replays or importing hidden ledger-only audit rows, and inbound `SYNC_HELLO` now triggers the same catch-up family set as `Sync All` for the receiver's local access profile.
...
- `tests/spec/bank_ledger_spec.lua`, `tests/spec/bank_ledger_scanner_spec.lua`, `tests/spec/ui_bank_ledger_spec.lua`, and `tests/spec/ui_options_spec.lua` verify append-only guild-bank log delta capture, repeated scans of the same visible log window not duplicating prior rows, preserving a legitimately new leading row even when it matches an older row exactly, tolerating rows with missing explicit date parts, `GuildBankLedger`-style session batch-count merging for shrinking/regrowing same-identity batches, remote money-delta replay not reintroducing the same repeated visible row on the next local scan, remote ledger sync sanitizing visible duplicate item or money rows from polluted peer caches before send or receive and refusing older-client visible duplicates that already exist on a repaired receiver even when the remote timestamp differs, legacy coarse money rows not reappending once a later local hour-precision Blizzard money-log scan sees the same row again, shifted legacy item rows not reappending once a later item-log scan sees the same visible date/item identity again, review-first ledger dedupe planning or apply for same-visible-date duplicate item rows plus duplicate money rows that match the visible ledger date, actor, action, and amount, source-stable money dedupe survivor selection so a cleanup pass does not reimport the current Blizzard money row on the next scan, a scrollable ledger-dedupe review modal for long cleanup lists, event-driven ledger log querying across all accessible item tabs plus the fixed Blizzard money-log slot, scanner-driven fully rotated item-log and money-log windows appending instead of getting stuck behind stale no-overlap source snapshots, debounced `GUILDBANKLOG_UPDATE` handling for delayed money-log rows, live `GUILDBANKLOG_UPDATE` wakeups that queue or start silent passive ledger scans without racing the main inventory scan, stable visible-tab behavior during ledger imports, automatic guild-scoped version-stamped `LEDGER_DELTA` publish for newly merged scanner rows without rebroadcasting no-change reruns, configurable repair-threshold classification, the shared configurable `Scan Interval` throttling both guild-bank auto-scan and ledger rescans, the newer player-interaction open/hide event path for passive rescans, reload-safe passive rescans when the bank is already open after `/reload`, active `Bank Ledger` table refresh when passive imports add rows, direct ledger rescans when the inventory snapshot is already fresh, handoff from the main guild-bank scan into ledger capture, retention pruning for ledger item/money rows and audit history, ledger CSV export with readable timestamps, item-vs-money table modes, real dropdown-backed action filtering, and user/item summary reporting. Stale historical ledger-cache recovery is no longer a required contract; this version intentionally clears ledger data once on first load, and `Options -> Data -> Dedupe Ledger` remains available for duplicate review cleanup if older local data returns.
...
7. For the current ledger-sync cleanup slice, also run a ledger replay check: after installing this version and reloading once, confirm the Bank Ledger starts from the one-time cleared baseline, then accept or send a remote ledger sync from a same-version or newer client and confirm a repaired receiver does not gain duplicate visible money or item rows. Also confirm a missing-version or older-version ledger sync is ignored and the next local bank-open money-log scan does not append an already-known Blizzard money-log row again. If older duplicate ledger rows already exist, open `Options -> Data -> Dedupe Ledger`, use `Review Rows`, confirm the review list scrolls when it overflows, and confirm only duplicate same-visible-date item rows or money rows with the same visible ledger date, actor, action, and amount are removed while a source-stable money row is kept when the current source snapshot can identify one.
