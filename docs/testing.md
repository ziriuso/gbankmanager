...
- `unit`
  ...
  - guild-scoped request routing, including request-management broadcasts that no longer whisper resolved roster recipients, officer-authoritative minimum snapshot acceptance or rejection, digest-first ledger sync coverage that suppresses repeated same-hash delta bursts, presence-only guild hello handling, remote ledger-delta merge coverage that preserves local scan freshness, only emits ledger chat when new rows are written, and rejects visible duplicate rows from polluted peer caches before they can match either the remote batch or already-repaired receiver rows, persisted peer-history buckets keyed by guild, and the retired auth-policy addon-comm path staying disabled
...
- `live smoke`
  ...
  - sync should report compact chat feedback for accepted incoming updates without turning the chat frame into a step-by-step spam log, while self login hello stays silent and ledger reject/no-change details remain available through `/gbm debug sync`
...
- `tests/spec/sync_spec.lua` now also verifies duplicate remote ledger deltas that merge zero new rows are still accepted for peer bookkeeping but do not add another `Synced ledger delta ...` chat line, that accepted remote ledger deltas still print one compact line when they actually merge new rows, and that ledger deltas without a same-or-newer addon version are rejected silently before they can mutate the repaired ledger.
