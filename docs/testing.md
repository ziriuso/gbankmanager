...
- `tests/spec/sync_spec.lua` now also verifies sync hello traffic and request snapshots can bootstrap a client whose root still points at the placeholder `Unknown Guild`, so the live guild key is promoted before peers are filed or request snapshots are rejected as `wrong_guild`.
