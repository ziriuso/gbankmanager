...
- `tests/spec/sync_spec.lua` now also verifies accepted remote request snapshots reconstruct the existing local `REQUEST_*` History rows when a snapshot is what catches a client up, accepted remote minimum snapshots reconstruct the existing local `MINIMUM_*` History rows while staying quiet on identical replays, and accepted remote `HISTORY_SNAPSHOT` payloads merge only the visible History-tab rows without duplicating identical replays or importing hidden ledger-only audit rows.
