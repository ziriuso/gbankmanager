...
- `tests/spec/bank_ledger_spec.lua` verifies raw relative Blizzard money-log rows (`0/0/0` plus visible hour) do not duplicate when later scans or sync payloads mint different absolute timestamps for the same visible money entry, and that `Options -> Data -> Dedupe Ledger` flags existing duplicate relative money rows for review cleanup.
