...
- `unit`
  ...
  - ambient NPC chat-filter coverage for normal Silvermoon Citizen suppression plus fail-open handling when WoW marks the sender, message, or rendered bubble text inaccessible
...
- `live smoke`
  ...
  - Switching bank tabs or log tabs while the same guild bank window remains open should not start another inventory auto-scan after the configured scan interval elapses; manual `Scan Bank` should still start immediately.
  ...
  - sync should report compact chat feedback for accepted incoming updates without turning the chat frame into a step-by-step spam log, while login-triggered catch-up stays silent and ledger reject/no-change details remain available through `/gbm debug sync`; inbound permission decisions should reflect the receiving client's local guild roster/rank view rather than trusting rank or guild-master flags in the remote payload
...
- `tests/spec/exports_spec.lua` and `tests/spec/ui_exports_spec.lua` also verify Auctionator's current caret-delimited import format plus the manual shopping list's line-broken local-only and Auction House guidance, reusable overflow scrollbar, Blizzard-standard bottom-right resize grip, responsive single-line row and viewport sizing, zero-buy `Restock from <Bank Tab>` label, shift-click Auction House search fill, checked-row bottom grouping, clearer built-in checkboxes, and canonical single-silver or gold-pentagon two-rank quality icons.
...
- `tests/spec/slash_commands_spec.lua` also enforces the taint-safety boundary for slash registration: GBankManager may add its own `SlashCmdList.GBANKMANAGER` entry but must never assign Blizzard's global `SlashCmdList` binding, which would taint protected Blizzard commands such as `/tm` and `/targetmarker`.
- `tests/spec/chat_filters_spec.lua` simulates WoW's `canaccessvalue` boundary and verifies inaccessible monster-chat senders fail open before the muted-NPC table lookup, inaccessible messages are not queued, inaccessible rendered bubble text is not matched or hidden, and accessible Silvermoon Citizen bubbles retain their existing suppression behavior.
...
- `tests/spec/sync_ledger_digest_spec.lua` keeps legacy digest handler coverage while asserting manual `Sync Ledger` now sends only the compact `LEDGER_MANIFEST` announcement until peers request buckets.
...
- `tests/spec/sync_spec.lua`, `tests/spec/sync_manual_actions_spec.lua`, `tests/spec/diff_spec.lua`, and `tests/spec/ui_minimums_sync_spec.lua` verify minimum deletions create and publish timestamped tombstones, stale peer snapshots cannot resurrect deleted rules, reciprocal catch-up includes deletion state, and a newer intentional rule can replace an older tombstone.
- `tests/spec/ui_minimums_sync_spec.lua` verifies `Minimums -> Save All` publishes the guild-scoped `MINIMUMS_SNAPSHOT` message family through addon communication.
...
- `tests/spec/ui_exports_spec.lua` verifies the floating manual shopping list stays independent from the main shell, remembers its saved position, grows through its standard bottom-right hash grip with bounded responsive rows and scrolling, and keeps normalized low-tier crafted icons in fallback rows.
