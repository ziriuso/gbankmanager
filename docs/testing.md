...
- `tests/spec/bank_ledger_spec.lua` verifies raw relative Blizzard money-log rows (`0/0/0` plus visible hour) do not duplicate when later scans or sync payloads mint different absolute timestamps for the same visible money entry, and that the internal ledger dedupe planner still flags existing duplicate relative money rows for cleanup coverage while the visible Data-tab action remains hidden.
- `tests/spec/store_spec.lua` verifies the v1.2.3 load-time money-ledger cleanup removes duplicate raw-relative money rows once, preserves item ledger rows, clears polluted money caches and stale ledger sync debug state, and records `meta.moneyLedgerDedupedForVersion = 1.2.3-money-v3`. It also verifies the v1.2.3 SavedVariables compaction pass removes generated snapshot search catalogs, preserves durable inventory rows, and records `meta.savedVariablesCompactedForVersion = 1.2.3-snapshot-v1`.
