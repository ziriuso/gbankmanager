...
- `tests/spec/sync_spec.lua` now also verifies duplicate remote ledger deltas that merge zero new rows are still accepted for peer bookkeeping but do not add another `Synced ledger delta ...` chat line.
...
7. For the current ledger-sync cleanup slice, also run a ledger replay check: accept or send a remote ledger sync from a client that still has duplicate visible money or item rows, then confirm a repaired receiver does not gain those duplicates and the next local bank-open money-log scan does not append an already-known Blizzard money-log row again. If older duplicate ledger rows already exist, open `Options -> Data -> Dedupe Ledger`, use `Review Rows`, confirm the review list scrolls when it overflows, and confirm only duplicate same-minute item rows or money rows with the same visible ledger date, actor, action, and amount are removed while a source-stable money row is kept when the current source snapshot can identify one.
