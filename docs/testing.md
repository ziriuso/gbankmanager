...
- `live smoke`
  ...
  - `Options -> Data` should let you open real dropdown menus for guild-bank ledger retention, audit-history retention, and the guild-bank `Scan Interval`, should expose the repair-threshold input plus the `Mute Silvermoon Citizen` toggle, should keep the save button inside the panel chrome, should show visible save feedback after `Save Settings`, should surface a `Clear Data` section with confirmation-gated destructive actions, and saving those settings should prune out-of-window Bank Ledger item/money rows plus audit-history rows when retention is finite while leaving inventory snapshots, Minimums, Requests, and sync peers alone. The mute toggle should persist after `/reload`.
