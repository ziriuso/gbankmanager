...
- `tests/spec/exports_spec.lua` and `tests/spec/ui_exports_spec.lua` also verify Auctionator's current caret-delimited import format plus the manual shopping list's line-broken local-only and Auction House guidance, reusable overflow scrollbar, zero-buy `Restock from <Bank Tab>` label, shift-click Auction House search fill, checked-row bottom grouping, clearer built-in checkboxes, and canonical single-silver or gold-pentagon two-rank quality icons.
