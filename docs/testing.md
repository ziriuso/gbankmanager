...
- `tests/spec/ui_options_spec.lua` now also verifies the `Options -> Sync` table refreshes immediately when new guild hello traffic arrives while the Sync tab is already open.
