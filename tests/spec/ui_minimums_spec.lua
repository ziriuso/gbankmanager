...
assert.truthy(mainFrame.minimumImportInputSurface ~= nil, "minimum import should wrap the editable payload field in a visible dedicated input surface")
assert.truthy(type(mainFrame.minimumImportInstructionText) == "table", "minimum import should show persistent helper copy above the paste field")
assert.truthy(string.find(tostring(mainFrame.minimumImportInstructionText:GetText() or ""), "Paste", 1, true) ~= nil, "minimum import helper copy should explain that the user should paste the import string into the field")
mainFrame.minimumImportModal:Hide()
mainFrame.minimumImportButton:GetScript("OnClick")(mainFrame.minimumImportButton)
assert.truthy(mainFrame.minimumImportInput:HasFocus(), "minimum import should focus the payload field when the modal opens so paste works immediately")
assert.truthy(not mainFrame.minimumImportReviewViewport:IsShown(), "minimum import should keep the review viewport hidden until a preview succeeds")
...
mainFrame:PreviewImportedMinimums("{")
assert.truthy(not mainFrame.minimumImportReviewViewport:IsShown(), "minimum import should keep the empty review viewport hidden after a parse error")
assert.truthy(string.find(tostring(mainFrame.minimumImportStatusText:GetText() or ""), "MinimumsPortability.lua", 1, true) == nil, "minimum import should surface a clean parser message instead of a raw Lua file path")
assert.truthy(tostring(mainFrame.minimumImportStatusText:GetText() or "") ~= "", "minimum import should still surface a visible parser message after invalid pasted JSON")
...
assert.truthy(mainFrame.minimumImportReviewViewport:IsShown(), "minimum import should reveal the review viewport once preview rows exist")
