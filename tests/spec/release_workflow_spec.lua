...
assert.truthy(string.find(workflow, '"forever-v*"', 1, true) ~= nil, "Forever release workflow should listen only for Forever version tags")
assert.truthy(string.find(workflow, "CF_FOREVER_GAME_VERSION_IDS", 1, true) ~= nil, "Forever upload should use a separate game-version override")
assert.truthy(string.find(workflow, "-Target Forever", 1, true) ~= nil, "Forever release workflow should select the Forever package and version resolver")
assert.truthy(string.find(workflow, "Assert-ForeverPackage.ps1", 1, true) ~= nil, "Forever release workflow should verify the exact zip before upload")
...
assert.truthy(string.find(buildScript, "Forever/GBankManager_ItemData", 1, true) ~= nil, "Forever package should select the Forever item database")
assert.truthy(string.find(buildScript, "forever-v", 1, true) ~= nil, "Forever package should have a separate tag prefix")
assert.truthy(string.find(buildScript, "16001", 1, true) ~= nil, "Forever package should stage a Forever-only main TOC")
