...
assert.equal(7, #(mainFrame.optionsTabButtons or {}), "options should expose seven top-level tab buttons")
...
assert.equal("Automation", ((mainFrame.optionsTabButtons or {})[5] or {}).labelText:GetText(), "options should expose an Automation tab")
assert.equal("Exports", ((mainFrame.optionsTabButtons or {})[6] or {}).labelText:GetText(), "options should expose an Exports tab")
assert.equal("Requests", ((mainFrame.optionsTabButtons or {})[7] or {}).labelText:GetText(), "options should expose a Requests tab")
...
assert.truthy((mainFrame.optionsAppearancePanel:GetHeight() or 0) <= 190, "appearance panel should not leave a large empty ghost box below the appearance controls")
...
assert.same((mainFrame.optionsThemeButtons or {}).pride, (mainFrame.optionsShellScaleLabel.points[1] or {})[2], "UI Scale should anchor beneath the last visible theme-button row instead of being buried under it")
...
assert.same(mainFrame.optionsShellScaleLabel, (mainFrame.optionsMinimapToggle.points[1] or {})[2], "minimap toggle should continue to flow from the UI Scale section after the appearance relayout")
