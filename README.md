...
- Ledger sync now announces a compact `LEDGER_DIGEST` before row deltas and suppresses repeated same-hash delta bursts for a short window, so busy live guilds can converge without echoing identical ledger rows back and forth. Duplicate remote ledger deltas that merge zero new rows also stay quiet in chat, so repeated no-op ledger sync traffic no longer spams `Synced ledger delta ...` lines.
...
- Visible `History` rows now have their own `HISTORY_SNAPSHOT` sync family, `Options -> Sync -> Sync History`, inclusion in `Sync All`, and guild hello now triggers the same catch-up family set as `Sync All` for the local access profile so reloaded clients can repopulate Requests, Minimums, History, and ledger state without a separate manual sync pass.
