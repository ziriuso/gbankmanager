...
- The v1.2.3 money-ledger guard runs once for clients that have not yet recorded `meta.moneyLedgerDedupedForVersion = 1.2.3-money-v3`; it removes duplicate money-log rows created from the same visible Blizzard money entry, clears polluted money-ledger caches, and preserves item ledger rows.
- The v1.2.3 SavedVariables compaction guard records `meta.savedVariablesCompactedForVersion = 1.2.3-snapshot-v1` after removing generated snapshot `searchCatalog` tables. Requests and Minimums build those search catalogs in memory so saved inventory snapshots keep only durable scan data.
...
- Ledger retention and History retention are active cleanup settings: finite values prune Bank Ledger item/money rows and visible audit-history rows during database load, after ledger scans, and when logs/history settings are saved. They do not currently prune inventory snapshots, Minimums, Requests, or sync-peer history.
