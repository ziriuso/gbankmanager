...
- The manual Shopping List now clips long sessions behind a slim scrollbar, labels zero-buy excess-stock rows as `Restock from <Bank Tab>`, fills the visible Blizzard Auction House Buy search when an item name is shift-clicked, and keeps the next unfinished item in place by moving checked rows to the bottom.
