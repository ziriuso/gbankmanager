...
- Visible `History` rows now have their own `HISTORY_SNAPSHOT` sync family, `Options -> Sync -> Sync History`, inclusion in `Sync All`, and automatic history snapshot publish on guild hello plus local request/minimum publishes so the `History` tab can converge directly across clients.
