...
- `Bank Ledger` now centralizes guild-bank item logs and money logs, stores append-only deltas from guild-bank log scans, avoids duplicating rows when repeated scans reread the same visible Blizzard log window, queries all accessible item logs plus the fixed Blizzard money-log slot in one burst, debounces `GUILDBANKLOG_UPDATE` before reading the logs back, keeps the visible guild-bank tab stable during import, passively rechecks for new log rows while the guild bank stays open, normalizes Blizzard’s relative log timestamps against server time, keeps ledger dedupe stable across `/reload` and relog by fingerprinting against computed absolute timestamps instead of drifting raw offsets, supports shared table filters plus a preset date-range dropdown, exposes ledger CSV export, shows item-log rows as `Date`, `Who`, `Action`, icon-based `Tier`, `Item`, `Quantity`, `Tab`, and `Moved From`, and summarizes item movement plus gold in, gold out, and repairs over the active date range
...
- Item-aware crafted-quality rendering now treats the bundled `GBankManager_ItemData` payload as the sole authoritative source for profession-tier family size and final non-inventory display-atlas choice. Inventory remains the source-of-truth presentation for bundled two-rank items, and the shared non-inventory surfaces now follow that same bundled display-atlas family instead of drifting back to the stale compact inline fallbacks or depending on live `C_TradeSkillUI` responses.
- `/gbm debug quality <itemID>` now prints the bundled crafted-quality rank, family size, bundled display or preferred atlas, any live reagent-quality payload, and the final atlas chosen by the shared item-aware display resolver for non-inventory surfaces.
...
1. Finish the remaining crafted-quality icon issue in non-inventory surfaces with one shared resolver contract, rather than more per-view atlas patches.
2. Revisit deeper sync catch-up so Requests, History, Bank Ledger, Minimums, and Inventory can sync cleanly with visible peer status and force-sync controls.
...
- Crafted-quality icons are still wrong outside Inventory, including `Minimums`, `Requests`, `Request Details`, request search results, and `Exports`.
- Passive `Bank Ledger` refresh still needs to match the working `GuildBankLedger` open-bank rescan behavior.
...
- `/gbm request` now opens a smaller end-user workflow window with `Guild Bank Manager` in the header, a four-column own-request status table, row-click details, request cancellation for pending own requests, and a three-step `New Request` wizard with a progress rail, preview card, quantity steppers, and quantity/reason confirmation flow.
...
- Crafted-quality rendering now normalizes the low-tier and max-tier atlas variants so Exports, Inventory, Minimums, Requests, and request-details all show the same visible quality symbols from the bundled addon data path.
- Crafted-quality rendering now splits cleanly by surface type: Inventory keeps its existing shared table treatment, while non-inventory surfaces such as Minimums, Requests, Exports, request details, and search selectors prefer dedicated texture-backed atlas regions or shared-table icon slots instead of relying on inline FontString atlas markup. Bundled item-data rank, family size, and display-atlas choice stay authoritative for those surfaces.
