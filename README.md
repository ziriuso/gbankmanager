...
- Guild sync foundation with authority-first conflict resolution, canonical `Character-Server` peer identity, compact inbound sync chat milestones, a dedicated visible `HISTORY_SNAPSHOT` sync family, and reconstructed or directly merged local History rows that already belong in the History tab
...
- The v1.1.0 compatibility guard clears stored Bank Ledger data once on first load for this version, records `meta.ledgerClearedForVersion`, stamps outbound ledger sync payloads with the addon version, and accepts inbound `LEDGER_DELTA` payloads only from same-version or newer clients so older polluted caches cannot repopulate repaired ledgers.
