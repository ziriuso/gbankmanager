...
- The v1.2.3 money-ledger guard runs once for clients that have not yet recorded `meta.moneyLedgerDedupedForVersion = 1.2.3-money-v4`; it removes duplicate money-log rows created from the same visible Blizzard money entry, clears polluted money-ledger caches, preserves item ledger rows, and local money scans reject drifted raw-relative replays before they can regrow duplicates.
...
- Routine scan and accepted-sync chat chatter is muted by default through `Options -> Appearance -> Suppress Routine Chat`; warnings, errors, explicit debug slash output, and in-game test output remain visible.
