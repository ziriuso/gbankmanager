...
- The v1.2.0 compatibility guard clears stored Bank Ledger data and ledger sync metadata once for clients that have not yet recorded `meta.ledgerClearedForVersion = 1.2.0`, and accepts inbound manifest or bucket ledger payloads only when they carry `ledgerProtocol = 2` or newer.
- The v1.2.3 money-ledger guard runs once for clients that have not yet recorded `meta.moneyLedgerDedupedForVersion = 1.2.3-money-v2`; it removes duplicate money-log rows created from the same visible Blizzard money entry, clears polluted money-ledger caches, and preserves item ledger rows.
