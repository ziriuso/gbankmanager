...
For local maintainer refresh or deploy work, start with [docs/maintainer-catalog-workflow.md](docs/maintainer-catalog-workflow.md). That doc now includes the normal Retail fast path, including the repo deploy helper and the expected Retail AddOns target on this machine.
...
- `Options` now also includes a dedicated `Data` tab that controls guild-bank ledger retention, audit-history retention, the configurable `Scan Interval`, and a repair-classification threshold for money-log withdrawals. Saving that panel now gives visible confirmation, that one scan interval now drives both the guild-bank-open auto-scan throttle and direct ledger rescans when the inventory snapshot is still fresh, and that same panel now owns irreversible local-data cleanup actions behind confirmation prompts. `Mute Silvermoon Citizen` and `Suppress Chat Except Sync Changes` now live under `Options -> Appearance` and save immediately when toggled, `Suppress Chat Except Sync Changes` defaults on for quieter installs, and `Repair Threshold` now explains that withdrawals equal to or under the value count as repairs instead of normal withdrawals.
- Dashboard `Top 10 Most Used` is now driven by bank-ledger withdrawal rows instead of the older stocking-history fallback.
- Recurring stock minimum helpers with a modal-based add/edit flow, portable export/import payloads, a review-first import editor with tab remapping, and procurement-planning export workflows
...
- Guild sync foundation with authority-first conflict resolution, canonical `Character-Server` peer identity, compact inbound sync chat milestones that can be muted through the global routine-chat option, manifest-first bank-ledger convergence, a dedicated visible `HISTORY_SNAPSHOT` sync family, and reconstructed or directly merged local History rows that already belong in the History tab
- `Bank Ledger` now centralizes guild-bank item logs and money logs, stores append-only rows from guild-bank log scans, avoids duplicating rows when repeated scans reread the same visible Blizzard log window, queries all accessible item logs plus the fixed Blizzard money-log slot in one burst, debounces `GUILDBANKLOG_UPDATE` before reading the logs back, keeps the visible guild-bank tab stable during import, prevents ledger starts during the main inventory scan, hard-defers direct ledger scan requests while inventory scanning is active, normalizes Blizzard's relative log timestamps against server time, uses durable count-based row identity for repeated same-hour activity, keeps scanner-driven fully rotated busy item-log and money-log windows instead of discarding them as suspicious no-overlap batches, and syncs through the 1.2.0 ledger protocol. Native scans publish compact `LEDGER_MANIFEST` messages only when new local rows are written; peers request only differing six-hour buckets, reply with bucket rows, fuller peers can push local bucket rows back to stale manifest senders, reject old or missing ledger protocols before merge, and print routine ledger sync feedback only when bucket replies actually append rows. The 1.2.0 compatibility boundary intentionally clears stored Bank Ledger rows and ledger sync metadata once while preserving inventory, Minimums, Requests, auth, blacklist, UI settings, and general sync peers. The page still supports shared table filters, preset date ranges, readable CSV export, item-vs-money modes, summaries, and retention; duplicate money-ledger cleanup is handled by versioned load-time cleanup instead of a visible Data-tab button.
- Local appearance customization with a token-backed theme system (`Default`, `High Contrast`, `Alliance`, `Horde`, `Legion`, `Nature`, `Pride`, `Void`), built-in WoW `UISliderTemplate` controls for `UI Scale` (90%-120%), shell opacity, and modal opacity, a right-column slider stack with matched slider widths, theme presets with the minimap toggle directly beneath them, collapsed-nav icons, a show/hide minimap-button toggle, a `GuildBankManager` minimap hover tooltip, LibDBIcon-style minimap-ring placement, direct slider interaction plus steppers, Escape-to-close shell handling that leaves the independent manual shopping list open and close-X-only, and a header last-scan display that now uses timezone abbreviations such as `EDT` or `EST` when a scan exists
...
- Taint-safe `/gbm` registration that adds only GBankManager's command entry and preserves Blizzard's global slash-command registry binding, so protected Blizzard commands such as `/tm` and `/targetmarker` retain their secure execution path
- Secret-safe ambient chat filtering that checks whether WoW-provided sender, message, and rendered bubble text can be accessed before comparing, formatting, or storing it; inaccessible chat fails open and remains visible

## Architecture

- `Core/` keeps bootstrap, constants, shared chat output, slash commands, and thin event registration.
...
- Minimums add flow now moves from the search modal directly into a centered `Minimum Details` modal, preserves the typed `Minimum` value when handing off into details, keeps the lower controls anchored in place when a crafted-quality item is selected, existing Minimums rows reuse that same modal with the current Bank Tab prefilled as a read-only value, and the Minimums table defaults to `Show All` on open.
- Minimums also supports a portable JSON export/import path. Import is review-first: pasted rows are parsed into a staging list, officers can edit quantity and enabled state, switch the local Bank Tab before saving, and rows with missing tabs stay blocked until they are mapped.
- Minimums and Requests both use the same reusable selector with clearly labeled `Search Item ID` and `Search Item Name` inputs plus one shared selected-item display that prefers trusted hyperlink-style item text and falls back to normalized plain names when no stored link is available. The selector-local crafted-quality icon path now stays enabled in both surfaces, so duplicate-name variants remain visually distinguishable in the result rows and selected-item summary.
...
- `Options -> Sync` now also repaints its peer table live while the Sync tab is already open, so incoming hello or accepted sync traffic can add or update peer rows without requiring the user to switch tabs away and back.
- Bank Ledger sync now uses compact bucket manifests, bucket requests, and bucket replies instead of row-delta bursts. `/gbm debug sync` records manifest, request, and reply decisions, while `/gbm debug ledger` includes the active ledger protocol, reset marker, global hash, bucket count, and recent bucket-sync state.
- The v1.2.0 compatibility guard clears stored Bank Ledger data and ledger sync metadata once for clients that have not yet recorded `meta.ledgerClearedForVersion = 1.2.0`, while the current 1.2.3 follow-up sync protocol accepts inbound ledger manifest, bucket, and delta payloads only when they carry `ledgerProtocol = 3` or newer.
- The v1.2.3 money-ledger guard runs once for clients that have not yet recorded `meta.moneyLedgerDedupedForVersion = 1.2.3-money-v7`; it removes duplicate money-log rows created from the same visible Blizzard money entry, clears polluted money-ledger caches, preserves item ledger rows, and local money scans reject short-window drifted raw-relative deposit replays before they can regrow duplicates. The v7 marker intentionally reruns recovery for clients that may have stamped v6 while still carrying Client-A-shaped drifted deposit duplicates.
- The v1.2.3 SavedVariables compaction guard records `meta.savedVariablesCompactedForVersion = 1.2.3-snapshot-v3` after removing generated snapshot `searchCatalog` tables, pruning inventory snapshots to the active snapshot plus two recent backups, and capping raw inventory diff diagnostics to the newest 500 rows. Requests and Minimums build search catalogs in memory so saved inventory snapshots keep only durable scan data.
- Closed-bank reload or stale guild-bank tab events no longer arm an auto-scan by themselves; inventory scans now wait for a real guild-bank-open state instead of starting timeout-driven scans against a closed bank after `/reload`. While the same guild bank window remains open, switching bank tabs or log tabs also no longer starts another inventory auto-scan after the scan interval elapses; manual `Scan Bank` remains immediate.
...
2. Tackle sync UX backlog polish such as active-view refresh for accepted remote sync updates and any future History sync design.
...
- Live/manual sync sanity has now passed in a real guild; current sync follow-up now includes the dedicated visible-history sync family, `Sync History`, and active-view refresh for accepted remote History, Request, and Minimum updates.
- Routine scan and no-change sync chatter is muted by default through `Options -> Appearance -> Suppress Chat Except Sync Changes`; sync chat is reserved for payloads that actually add or change local rows, while warnings, errors, explicit debug slash output, and in-game test output remain visible.
...
- The manual Shopping List now clips long sessions behind a slim scrollbar, uses Blizzard's standard bottom-right hash grip to expand its single-line rows and scroll viewport, labels zero-buy excess-stock rows as `Restock from <Bank Tab>`, fills the visible Blizzard Auction House Buy search when an item name is shift-clicked, and keeps the next unfinished item in place by moving checked rows to the bottom.
...
- Accepted remote `REQUESTS_SNAPSHOT` and `MINIMUMS_SNAPSHOT` payloads now reconstruct the same existing request and minimum History row types locally, so catch-up sync can repopulate the `History` tab without inventing new ledger- or snapshot-only audit categories.
- Minimum removals now persist and sync timestamped deletion tombstones, so an older peer snapshot cannot repeatedly restore obsolete Minimums rules; a genuinely newer add or edit still replaces the older deletion marker.
- Visible `History` rows now have their own `HISTORY_SNAPSHOT` sync family, `Options -> Sync -> Sync History`, and inclusion in `Sync All`. Guild hello is presence-only: it updates peer last-seen state without replaying the `Sync All` family set, while manual sync actions and native local writes still publish the relevant request, minimum, history, or ledger payloads.
...
- Ledger retention and History retention are active cleanup settings: finite values prune Bank Ledger item/money rows and visible audit-history rows during database load, after ledger scans, and when logs/history settings are saved. Inventory snapshots are separately compacted to the active snapshot plus two recent backups; Minimums, Requests, and sync-peer history are not pruned by those retention settings.
