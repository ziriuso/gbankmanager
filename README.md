...
- Guild sync foundation with authority-first conflict resolution, compact inbound sync chat milestones, and reconstructed local History rows for accepted remote request/minimum changes that already belong in the History tab
...
- Accepted remote `MINIMUMS_SNAPSHOT` payloads now also reconstruct the same existing `MINIMUM_CREATED`, `MINIMUM_UPDATED`, `MINIMUM_ENABLED`, `MINIMUM_DISABLED`, and `MINIMUM_REMOVED` History rows locally by diffing the prior and accepted minimum rules, so the History tab stays aligned across clients without inventing new ledger- or snapshot-only audit categories.
